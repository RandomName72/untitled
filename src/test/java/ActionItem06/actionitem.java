...
// store studio links in arraylist of webelements
    ...
    public static void clickByIndex(WebDriver driver, int index) {
        ArrayList<WebElement> studioLink = new ArrayList<>(driver.findElements(By.cssSelector(".linkContainer-C5n9z")));
        studioLink.get(index).click();
    }
    public static void scrollIntoView(WebDriver driver) {
        JavascriptExecutor jse = (JavascriptExecutor) driver;
        WebElement workshops = driver.findElement(By.cssSelector(".title-UbUc9"));
        jse.executeScript("arguments[0].scrollIntoView(true);", workshops);
    }
}
